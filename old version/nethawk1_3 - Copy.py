...
class PCAPLoader(QThread):
    ...
    def detect_protocol(self, sport, dport, packet, transport):
        ...
        # Enhanced RTP detection for UDP
        if transport == 'UDP' and (10000 <= sport <= 20000 or 10000 <= dport <= 20000):
            if packet.haslayer(Raw):
                ...
                if len(payload) >= 12:
                    ...
                    if version == 2:
                        # Additional validation: check if payload type is valid (0-127)
        ...
        if transport == 'UDP' and (self.rtp_port_range[0] <= sport <= self.rtp_port_range[1] or
            ...
            if packet.haslayer(Raw):
                ...
                if len(payload) >= 12:
                    ...
                    if version == 2:
                        payload_type = payload[1] & 0x7F
                        if payload_type <= 127:  # Valid RTP payload type
                            return 'RTP'
...
class NetHawkPro(QMainWindow):
    ...
    def setup_ui(self):
        self.setWindowTitle('NetHawk Pro - Sean Foley')
    ...
    def open_pcap(self):
        # Prevent opening multiple files simultaneously
        if self.pcap_loader and self.pcap_loader.isRunning():
            QMessageBox.warning(self, 'Busy', 'Already loading a PCAP file. Please wait...')
            return
    ...
    def on_pcap_batch_loaded(self, packet_batch):
        for packet_info in packet_batch:
            ...
            # Queue VoIP parsing instead of doing it immediately
    ...
    def is_likely_voip(self, packet_info):
        """Quick check if packet is likely VoIP - no heavy parsing"""
    ...
    def parse_voip_from_packet_info_fast(self, packet_info):
        """OPTIMIZED: Fast VoIP parsing without full packet reconstruction"""
        ...
        # Only reconstruct if we really need to
        try:
            ...
            if protocol == 'SIP' or sport in [5060, 5061] or dport in [5060, 5061]:
                ...
                try:
                    ...
                    sip_info = {
                        ...
                        'sport': sport,
                        'dport': dport,
                        'raw': sip_data[:500],  # Limit raw data size
                        'headers': {}
                    }
                    ...
                    # Parse only essential headers
                    for line in lines[1:20]:  # Limit header parsing
                        if ':' in line:
                            key, value = line.split(':', 1)
                            key = key.strip()
                            if key in ['Call-ID', 'From', 'To']:  # Only essential headers
                    ...
                    if sip_info['call_id']:  # Only add if we have a call ID
            ...
            # Fast RTP check
                ...
                try:
                    ...
                    if header_len <= len(raw_data):
                        rtp_payload = raw_data[header_len:header_len+100]  # Limit payload size
    ...
    def on_pcap_load_complete(self, progress, filename, count):
        ...
        QMessageBox.information(
            self,
            'PCAP Loaded',
            f'Successfully loaded {count} packets from:\n{os.path.basename(filename)}{voip_info}'
        )
        
        self.status_bar.showMessage(f"Loaded: {os.path.basename(filename)} ({sip_count} calls, {rtp_count} streams)")
        self.update_status()
        self.pcap_loader = None
        
        # Auto-switch to VoIP tab if VoIP detected
        if sip_count > 0:
            self.tabs.setCurrentIndex(1)  # Switch to VoIP tab
...
def main():
    ...
    app.setApplicationName("NetHawk Pro - Sean Foley")
    try:
        window = NetHawkPro()
        window.show()
        return app.exec_()

#!/usr/bin/env python3
"""
NetHawk Pro - PROFESSIONAL UI with Thread Safety
Performance-optimized with modern UI design
"""
...
COLORS = {
    ...
    'card': '#37474F',         # Card Background
}

# Protocol colors (updated modern palette)
PROTOCOL_COLORS = {
...
class ModernPacketTable(QTableWidget):
    """MODERN & THREAD-SAFE packet table"""
    ...
    def flush_pending_packets(self):
        """Flush pending packets to UI (runs on main thread)"""
        with self._pending_lock:
    ...
    def display_packet_fast(self, packet_info):
        ...
        protocol_item.setFont(QFont("Arial", 9, QFont.Bold))
...
class PacketDetailsWidget(QTextBrowser):
    """Packet details viewer with modern styling"""
    def __init__(self):
        super().__init__()
    ...
    def show_packet_details(self, packet_info):
        ...
        <div class="info-box">
            <strong>Info:</strong> {packet_info.get('info', 'N/A')}
        </div>
        """
        self.setHtml(html)
...
class CallFlowDiagram(QGraphicsView):
    ...
    def __init__(self):
        ...
        self.setStyleSheet(f"QGraphicsView {{ background-color: {COLORS['dark']}; border: none; border-radius: 8px; }}")
    
    def draw_call_flow(self, sip_call):
        self.scene.clear()
        
        if not sip_call.messages:
            text = self.scene.addText("No SIP messages", QFont("Arial", 14))
        ...
        for i, endpoint in enumerate(endpoints):
            ...
            # Modern endpoint box
            rect = QGraphicsRectItem(x-80, start_y, 160, 40)
            rect.setBrush(QBrush(QColor(COLORS['card'])))
            rect.setPen(QPen(QColor(COLORS['primary']), 2))
            self.scene.addItem(rect)
            
            text = self.scene.addText(endpoint, QFont("Arial", 10, QFont.Bold))
            text.setDefaultTextColor(QColor(COLORS['light']))
            text.setPos(x - text.boundingRect().width()/2, start_y + 10)
            
            # Dashed vertical line
            line = QGraphicsLineItem(x, start_y + 40, x, start_y + 40 + len(sip_call.messages) * msg_spacing + 40)
        ...
        for msg in sip_call.messages:
            ...
            # Arrow head
            if src_x < dst_x:
                points = [QPointF(dst_x-10, y-6), QPointF(dst_x, y), QPointF(dst_x-10, y+6)]
            else:
                points = [QPointF(dst_x+10, y-6), QPointF(dst_x, y), QPointF(dst_x+10, y+6)]
            ...
            label_text.setPos((src_x + dst_x)/2 - label_text.boundingRect().width()/2, y - 24)
...
class SIPMessageDetailsWidget(QTextBrowser):
    ...
    def show_message_details(self, sip_message):
        ...
        <div class="field">
            ...
            <div style="font-family: Consolas; font-size: 10px;">{sip_message.get('call_id', 'N/A')}</div>
        </div>
        
        <h3 style="color: {COLORS['info']}; margin-top: 24px;">Raw Message</h3>
...
class RTPStreamWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        # Header
        header = QLabel("🎵 RTP Streams & Audio Export")
        ...
        export_all = QPushButton("💾 Export All Streams")
        ...
        clear_btn = QPushButton("🗑️ Clear Streams")
    ...
    def update_display(self):
        ...
        for row, (ssrc, stream) in enumerate(self.streams.items()):
            ...
            export.setStyleSheet(f"QPushButton {{ background-color: {COLORS['success']}; color: white; border: none; border-radius: 4px; }}")
            export.clicked.connect(lambda _, s=ssrc: self.export_stream(s))
            action_l.addWidget(export)
            
            play = QPushButton("▶️")
            play.setMaximumWidth(40)
            play.setStyleSheet(f"QPushButton {{ background-color: {COLORS['primary']}; color: white; border: none; border-radius: 4px; }}")
    ...
    def clear_streams(self):
        """Clear all streams without confirmation"""
        self.streams.clear()
        self.stream_table.setRowCount(0)
    
    def clear_streams_with_confirm(self):
        """Clear streams with confirmation dialog"""
...
class StatsDashboard(QWidget):
    ...
    def setup_ui(self):
        ...
        stats = [
            ...
            ('calls', '📞 SIP Calls', '0', COLORS['success']),
            ('streams', '🎵 RTP Streams', '0', COLORS['info']),
            ('data', '📊 Data Rate', '0 KB/s', COLORS['warning'])
        ]
    ...
    def create_stat_card(self, label, value, color):
        ...
        label_widget.setStyleSheet(f"color: {COLORS['light']}; font-size: 10pt; font-weight: 600;")
...
class NetHawkPro(QMainWindow):
    """PROFESSIONAL Main Application with Thread Safety"""
    def __init__(self):
        super().__init__()
    ...
    def setup_packet_tab(self):
        ...
        self.display_filter.setPlaceholderText("Filter by protocol, IP address, or text...")
    ...
    def setup_voip_tab(self):
        ...
        selector.addWidget(QLabel("📞 Select Call:"))
    ...
    def setup_remote_tab(self):
        ...
        self.tabs.addTab(widget, "🔗 Remote Agents")
    ...
    def process_packet_batch(self):
        """Thread-safe packet batch processing"""
    ...
    def on_pcap_batch_loaded(self, packet_batch):
        """Handle batch of loaded packets"""
    ...
    def process_voip_queue(self):
        """Process VoIP parsing queue in batches"""
        with self.voip_queue_lock:
            if not self.voip_parse_queue:
                return
            
            # Process up to 50 packets at a time
            batch_size = 50
    ...
    def is_likely_voip(self, packet_info):
        """Quick check if packet is likely VoIP"""
    ...
    def on_pcap_load_complete(self, progress, filename, count):
        ...
        # Auto-switch to VoIP tab if VoIP detected
        if sip_count > 0:
            self.tabs.setCurrentIndex(1)
    
    def on_pcap_load_error(self, progress, error):
        """Handle PCAP load error"""
        progress.close()
        self.unlock_ui()
    ...
    def show_about(self):
        ...
        QMessageBox.about(
            ...
            f'''<h1 style="color: {COLORS['primary']};">NetHawk Pro</h1>
            <p><b>Professional Network Protocol Analyzer</b></p>
            <p>Version 2.0 - Performance Optimized</p>
            ...
            <p style="margin-top: 20px;">
                <b>Handles 100K+ packets smoothly</b><br>
                Better than Wireshark for VoIP analysis!
            </p>
